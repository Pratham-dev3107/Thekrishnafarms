# The Krishna Farms 🌿

> **Punjab's Premier Luxury Farmstay & Royal Wedding Destination**  
> Located on Bholath-Kala Bakra Road, Kala Bakra, near Jalandhar, Punjab 144303.  
> Direct Helpline / WhatsApp: **+91 94176 11073**

---

## 🏛️ About The Estate
The Krishna Farms is a luxury farm estate, grand wedding lawn, and authentic wooden farmstay chalet retreat.
- **Imperial Wedding Lawn**: 1,000+ guest capacity open-air lawn
- **Crystal Swimming Pool**: Poolside cabana deck for Haldi, Mehendi, and cocktail sundowners
- **Eco-Luxury Wooden Chalets**: Solid timber cottages starting from ₹3,990/night with modern AC & attached ensuite bathrooms
- **Location**: 20 minutes from Jalandhar Bypass / GT Road

---

## 🚀 Deployment (Render Static Site)
This is a high-performance, responsive static website built with pure HTML5, Vanilla CSS, and JavaScript.

- **Build Command**: *(leave empty)*
- **Publish Directory**: `.` *(root)*
- **Node/Python**: None required (pure static site)
